...
class BinarySearchTree:
    ...
    def insert(self, key: int, value: Any) -> None:
        ...
        if self._root is None:
            ...
            self._root = self._insert_new(self._root. key, value)

    def _insert_new(self, node: TreeNode, key: int, value: Any):
        if node is None:
            self._size += 1
        ...
        if key == node.key:
            ...
            node.left = self._insert_new(self._root. key, value)
        else:
            node.right = self._insert_new(self._root. key, value)
