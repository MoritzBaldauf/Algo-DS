...
class UnitTestSortedDoublyLinkedList(unittest.TestCase):
    def setUp(self):
        ...
        self.arr_feb = [2, 2, 3, 4, 6, 9, 16, 22]
    ...
    def test_invalid_search_value(self):
        with self.assertRaises(ValueError, msg="No ValueError for invalid input."):
            self.sorted_dll.search_value(2.5)
        with self.assertRaises(ValueError, msg="No ValueError for invalid input."):
            self.sorted_dll.search_value('a')
    ...
    def test_filter_even(self):
        ...
        self.assertEqual(0, len(self.empty_dll), "Size incorrect after filter even operation.")
        self.sorted_dll.filter_even()
        self.assertEqual("[2, 4]", str(self.sorted_dll), "Filter even operation failed.")
        self.assertEqual(2, len(self.sorted_dll), "Size incorrect after filter even operation.")
        self.sorted_dll = create_list_from_array(self.arr_feb)
        self.sorted_dll.filter_even()
        self.assertEqual("[2, 2, 4, 6, 16, 22]", str(self.sorted_dll), "Filter even operation failed.")
        self.assertEqual(6, len(self.sorted_dll), "Size incorrect after filter even operation.")
        self.sorted_dll = create_list_from_array(self.arr_odd)
        self.sorted_dll.filter_even()
        self.assertEqual("[]", str(self.sorted_dll), "Filter odd operation failed.")
        self.assertEqual(0, len(self.sorted_dll), "Size incorrect after filter even operation.")
        self.sorted_dll = create_list_from_array(self.arr_even)
        self.sorted_dll.filter_even()
